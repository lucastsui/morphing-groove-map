...
_Last updated: 2026-06-05 (Spark analysis made always-on: removed the Welcome "Use Spark"/server-URL section, hardcoded the Spark IP in `Store`, dropped UserDefaults persistence — verified the Spark path 7/7 via headless SELFTEST + live in the sim. Prior 2026-06-04: symlog offset bars/slider + extraction-accuracy test + ±6k slider scale + snap-to-grid + root-cause grid-crash hardening, all committed + pushed). This file is a self-handoff so a fresh context can resume fast._
...
- **App side**: `RemoteAnalyzer.swift` POSTs the song and decodes the envelope via `MGMIO.decodeSTT` (reuses the exact .stt validation; plus a `subdivision % denominator` crash-guard). `Store.analyzeSong` is **remote-first with automatic on-device fallback**. **Spark is now always-on with the IP hardcoded** in `Store` (`useRemote = true`, `serverURL = "http://100.73.106.98:8001"`) — the Welcome "Use Spark"/server-URL section was removed and the UserDefaults persistence dropped (it's a demo; security/config out of scope). `useRemote` stays a `var` (not `let`) so `StoreTests` can still force the deterministic on-device path. Generate tab shows the engine used (`Spark · htdemucs+librosa` vs `on-device`).
...
- `./run_tests.sh` → **ALL TIERS PASSED**: Tier-0 MGMValidate (~31 checks), Tier-1 MGMKit **52 tests**, Tier-2 Store **14 tests**, Tier-3 XCUITest **6 tests** (incl. crash-smoke).
...
- History: `8471947` (rewrite) → `781cc23` (Spark remote analysis + on-device full-song) → `0096f2f` (exact MIDI extraction + combined import button + Generate-tab groove editor + `MIDICompare`) → `7752889` (direction-aware strip gesture) → `22fc0a7` (CI fix) → `cbf9628` (lossless `ClipGroove` + `.agr` import) → `eb61306` (`.STT beats` always-editable + typeable note-count box; Edit button removed) → `8c75b8d` (.MGM slot-timeline full-width alignment — **teammate Lucas**) → `a61bece` (symlog offset bars + tall `.STT beats` `BeatSlider`, rebased on the teammate's commit) → `2ae4b98` (fix flaky `testMGMShowsDefaultSlots`) → `5e28453` (extraction-accuracy test + slider re-tune `k=6` [±6k = 2/3–3/4] + snap-to-grid) → `b3ed358` (crash fix: guard `assignCurrentToSlot`/`saveMGM` against an invalid grid; `testAssignToSlotGuardsInvalidGrid`) → `2cc129b` (max-effort code-review batch 1 — root-cause grid-crash hardening: clamp `beatResolution`/`timeSignature` to ≥1, always-valid `currentGroove` fallback, MGM strip-drag bounds-check, `saveMGM` derives the doc grid from the stored slots; retires the 2 confirmed crashes + the `.mgm` export failure + the altitude finding) → **latest** (Spark always-on: removed the Welcome server-URL field + "Use Spark" toggle, hardcoded the Spark IP in `Store`, dropped UserDefaults persistence; `useRemote` stays a `var` so the CI test can still force the on-device path — verified **7/7 via headless SELFTEST** and **live in the sim** on *Fly Me to the Moon* → **120 bpm** via `htdemucs+librosa`). All **pushed to `origin/main`**.
...
- `precondition` crash paths (e.g. `Render.grooved` bf-without-tempo) aren't unit-tested (XCTest can't catch a `precondition`). The **grid-input** ones are now defused at the root: `timeSignature` and `beatResolution` clamp to ≥1 and `currentGroove`'s fallback subdivision is grid-valid, so `slicesPerBeat` can't be reached with a bad subdivision via the UI (a max-effort code review surfaced these; remaining review findings — `STTFullView` still on the old linear scale, drag-area/sensitivity UX, `ExtractionAccuracyTests` vacuous velocity assert + greedy alignment, snap granularity — are NOT yet fixed).
